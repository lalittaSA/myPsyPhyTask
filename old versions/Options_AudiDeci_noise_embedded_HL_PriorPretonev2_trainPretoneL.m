function options = Options_AudiDeci_noise_embedded_HL_PriorPretonev2_trainPretoneL
%% frequency settings
...
%% pretone settings
options.nPreTones = 0:3;
options.preToneCombination = 'single'; % select from {'single','combine','permute'};


%% prior & amplitude settings
options.priorLevels = [-1]; % from {-3 -2 -1 0 1 2 3} | prior setting (arbitrary numbers -> used to set location of pre-cue)
...
options.snrPrior = [3; ... % low-freq trials
                    1];    % high-freq trials
