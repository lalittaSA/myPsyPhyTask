function [task, list] = AudiDeci_noise_embedded_HL_ampCalib_dB(dispInd,subjID,eyeTrackerOn)

% 20170905: calibrate in dB instead of voltage
...
if exist(calibFileName,'file')
    ...
    if ismember(hd.loFreq,calibrationFile.cf) && ismember(hd.hiFreq,calibrationFile.cf)
        dB_list = calibrationFile.targetLevels; % becomes calibrationFile.targetLevels after new calibration
        voltageLo = calibrationFile.calibratedamplitude(:,ismember(calibrationFile.cf,hd.loFreq));
        voltageHi = calibrationFile.calibratedamplitude(:,ismember(calibrationFile.cf,hd.hiFreq));
    else
...
if exist(calibNoiseFileName,'file')
    ...
    if ismember(hd.noiseAmp,calibrationFile.targetLevels)
        hd.noiseVolt = calibrationFile.calibratedVoltagesMean(ismember(calibrationFile.targetLevels,hd.noiseAmp));
    else
...
% if questVersion == 1
    tGuess = 0.75; %Guess at the appropriate amplitude
    tGuessSd = 0.4; %Standard deviation in guesses permitted
    
    pThreshold = 0.75; %How successful do you want a subject to be?
...
%     beta = 3.5;
    ...
    q = QuestCreate(tGuess, tGuessSd, pThreshold, beta, delta, gamma, 0.01, 0.4); %Quest object created
    
    q.normalizePdf = 1;
% initialize quest for high & low amplitude separately  
    list{'Quest'}{'ObjectH'} = q;
    list{'Quest'}{'ObjectL'} = q;
    list{'Quest'}{'VoltageL'} = voltageLo;
    list{'Quest'}{'VoltageH'} = voltageHi;
    list{'Quest'}{'dB'} = dB_list;
...
if questVersion == 1
    switch freqType
        case 'H'
            ...
            voltageList = list{'Quest'}{'VoltageL'};
        case 'L'
            q = list{'Quest'}{'ObjectL'};
            voltageList = list{'Quest'}{'VoltageH'};
    end
elseif questVersion == 2
    q = list{'Quest'}{'Object'};
end
...
dB_list = list{'Quest'}{'dB'};

% find closest calibrated amplitude
hd = list{'Stimulus'}{'header'};
ampTest = dB_list(knnsearch(dB_list',ampTest*100));
% ampTest = voltageList(knnsearch(voltageList,ampTest));
lastToneAmp = voltageList(dB_list == ampTest);

[~,waveform,f,h] = stimGen_noise_embedded_HL(hd.loFreq,hd.hiFreq,freqType,hd.toneDur,hd.toneIBI,0,0,lastToneAmp,hd.noiseVolt,hd.fs);
