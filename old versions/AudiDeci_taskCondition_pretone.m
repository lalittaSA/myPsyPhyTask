function list = AudiDeci_taskCondition_pretone(list,options)
...
expDecay = exp(-nPreTones/5.5); % to get mean pretone lenght ~5
%     plot(expDecay); % just for testing
nPreToneDistrib = nPreToneSeqType*expDecay/sum(expDecay);
sumDistrib = sum(nPreToneDistrib);
nPreToneDistrib = round(nPreToneDistrib*(nPreToneSeqType/sumDistrib));
