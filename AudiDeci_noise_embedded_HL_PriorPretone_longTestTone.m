function [task, list] = AudiDeci_noise_embedded_HL_PriorPretone_longTestTone(dispInd,ampRanges,subjId,optionName,eyeTrackerOn)
...
list = AudiDeci_taskCondition_hazard_new(list,options);
...
lastToneDur = list{'Stimulus'}{'lastToneDur'};
freqType = trialVarStimSeq{counter};
tic
[~,waveform,f,h] = stimGen_noise_embedded_HL_longTest(loFreq,hiFreq,freqType,toneDur,toneIBI,toneAmpL,toneAmpH,lastToneAmp,lastToneDur,fs);
toc
